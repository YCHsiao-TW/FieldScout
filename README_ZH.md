...
地圖 marker 與 occurrence card 會同步並持續高亮。為避免大量結果拖慢手機，清單與地圖每次顯示 200 筆，可按「載入更多紀錄」繼續加入；篩選、月份統計、候選排名、全部加入行程與 CSV／GeoJSON 匯出仍使用完整已載入結果。詳情視窗可查看原始圖片、metadata、授權、敏感資料標記、原始來源與 Google Maps。
