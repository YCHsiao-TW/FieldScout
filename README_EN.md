...
- Route reordering by north → south or current-GPS distance
- Top 5 candidate sites pinned before other candidates and raw occurrences
- Independent source health for TBIA, TBN, GBIF, and iNaturalist
- Failed TBIA/TBN requests no longer break the whole search; unavailable sources are hidden for that search
- Original-source thumbnails and image galleries where public media URLs are available
- Interactive monthly occurrence chart with visible counts
- Tapping a month applies or clears the corresponding Explore month filter

## v0.9.6 entry recovery
