...
- **完整 occurrence 搜尋**：整合 GBIF 分頁與 iNaturalist ID 接續讀取；FieldScout 不另設搜尋總筆數限制。臺灣工作空間另使用 TaiCOL 協助名稱解析。
- **進度、取消與可靠快取**：分別顯示兩個來源的讀取進度，可隨時取消；某一來源暫時故障時，可用該來源先前完整快取補足，且不會以不完整結果覆蓋完整快取。
- **完整地圖與分頁清單**：地圖在背景分批加入全部篩選點位；清單每頁 200 筆，可前後翻頁或輸入頁碼。點選地圖會切到對應清單頁，也能直接查看詳情。分頁不影響篩選、候選排名、全部加入與匯出的完整結果。
...
- **Field 模式**：逐點導航、GPS 距離、訪查狀態、行程進度、快速建立新的綁定紀錄；GPS Track 自動保存，重新開始會接續新段落，GPX 保留段落。
- **採集／觀察紀錄**：標本號自動編號、數量、物種、微棲地、方法、備註、現場或 Trip Point GPS、照片、編輯、刪除與 QC。可明確選擇所屬行程與採集點；照片、紀錄與訪查狀態以同一交易儲存，失敗時保留表單。
- **資料匯出**：occurrence、Trip 與 field records 的 CSV／GeoJSON／GPX；另有 deterministic 模糊座標 CSV。CSV 會中和試算表公式型內容。
- **完整備份與還原**：備份 settings、Trips、records 與照片，可選擇包含搜尋快取。大型備份自動分檔並校驗完整性；還原同時寫入全部資料。跨工作空間匯入會複製資料與關聯，不覆寫原工作空間。
...
- GPS 自動保存仍需要瀏覽器繼續執行；手機鎖屏或背景暫停可能造成定位中斷。若顯示待儲存或儲存失敗，請保留頁面並重試。
